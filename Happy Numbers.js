...
happy(107) ➞ false
Notes
You can assume (and it is actually true!) that all positive whole numbers are either happy or unhappy. Any happy number will have a 1 in its sequence, 
...
function happy(n) {
    do {
        n = [...String(n)].map(e => parseInt(e) ** 2).reduce((acc,vl) => acc + vl, 0)
        if(n === 4) return false;
    } while(n!== 1)
    return true
}

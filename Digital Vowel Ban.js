...
If n has at least a digit that contains the given vowel in its name, the new integer obtained after the elimination
of banned digits (as a natural number without leading zeros).
...
function digitalVowelBan(n, ban) {
    vowels = ['eo','oe','o','e','ou','ie','i','e','ei','ie']
    let str = ''
    let aux = [...n.toString()].map(e => {
        if(!vowels[parseInt(e)].includes(ban)) return str += e
    })

...
function cuttingGrass(arr, ...cuts) {
    ...
    for(let i=0;i<cuts.length;i++) {
        for(let k=0;k<arr.length;k++) {
            // console.log(arr[k], cuts[i]);
            if(arr.indexOf('Done') !== -1 ) arr[k] = "Done"
            if(arr[k] >= 1) arr[k] -= cuts[i]
            if(arr[k] < 1) { arr[k] = "Done"; }
        }
        collect.push(arr.slice())
    }
    
    // if(collect[cuts.length-1].every(e => e === 'Done')) return collect[cuts.length-1]

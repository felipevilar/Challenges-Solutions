...
The multiplicative persistence of an integer, n, is the number of times you have to replace n with the product of its digits until n 
...
function additivePersistence(n) {
    ...
    while(String(n).length !== 1) {
        n = [...String(n)].reduce((acc,el) => acc + parseInt(el), 0)
        count++
    }
    return count
}
...
function multiplicativePersistence(n) {
    let count = 0
    while(String(n).length !== 1) {
        n = [...String(n)].reduce((acc,el) => acc * parseInt(el), 1)
        count++
    }
    return count
}

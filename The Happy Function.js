/* 
In this challenge, you have to implement an algorithm to establish if a given positive integer num is a Happy number, and how many steps of 
the algorithm are needed to establish it.
...
If num is not a Happy number, the string "SAD x" with the "x" being the number of steps necessary to enter into the infinite loop 
reaching so any number in the series 4, 16, 37, 58, 89, 145, 42, 20, plus the number of steps necessary to obtain again that number.
...
*/
function happyOp(num) {
    let result = [...num.toString()].reduce((acc,el) => acc + parseInt(el)**2, 0)
    return result
}
...
function happyAlgorithm(num) {
    ...
    while(aux !== 1) {
        arr.push(aux)
        aux = happyOp(aux)
        if(arr.indexOf(aux) !== -1 || aux === num) return "SAD "+(arr.length+1)
    }

    return "HAPPY "+(arr.length+1)
}
